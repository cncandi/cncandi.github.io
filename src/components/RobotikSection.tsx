      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
        <div data-reveal="left">
          <div className="inline-flex items-center gap-2 mb-3">
            ...
            <span className="text-brand-teal text-xs font-bold tracking-widest uppercase">{lang==='de' ? 'Roboter-Bearbeitung' : 'Robot Machining'}</span>
          </div>
          ...
          <p className="text-brand-muted leading-relaxed mb-8">
            {lang==='de'
              ...
              : 'Program your robot offline — without downtime. ENCY and P3D Slicer support KUKA, FANUC, ABB, Yaskawa and more.'}
          </p>
          <div className="grid grid-cols-3 gap-3 mb-8">
            {apps.map((a,i) => (
              <div key={a.de} data-reveal data-delay={String(i+1)} className="group relative rounded-xl overflow-hidden aspect-square img-zoom">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-brand-navy/75 to-transparent group-hover:from-brand-teal/60 transition-all duration-300" />
                <div className="absolute bottom-2 inset-x-1 text-white text-xs font-bold text-center">{lang==='de' ? a.de : a.en}</div>
              </div>
            ))}
          </div>
          <Link href="/robotik/programmierung" className="btn-primary">
        ...
        <div data-reveal="right" className="space-y-4">
          <div className="rounded-2xl overflow-hidden shadow-2xl img-zoom border border-brand-teal/10">
            <Image src="/images/encycam/Build-Your-Robotic-Cell-Digital-Twin-scaled.jpg" alt="Digital Twin" width={640} height={380} className="w-full object-cover" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="rounded-xl overflow-hidden shadow-lg img-zoom">
              <Image src="/images/encycam/Full-robotic-cell-simulation.png" alt="Simulation" width={300} height={190} className="w-full object-cover" />
            </div>
          </div>
        </div>
      </div>
